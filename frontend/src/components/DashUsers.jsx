...
const DashUsers = () => {
  ...
  useEffect(() => {
    const fetch_users = async () => {
      try {
        ...
        const res = await fetch('https://https://mern-blog-digr.onrender.com/api/user/getusers',{
            method: 'GET',
            credentials: 'include',
            headers: {
  ...
  const delete_user = async (userid) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const res = await fetch(`https://https://mern-blog-digr.onrender.com/api/user/deleteuser/${userid}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${token}`,
