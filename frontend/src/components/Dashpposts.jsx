...
const Dashpposts = () => {
  ...
  useEffect(() => {
    const fetch_posts = async () => {
      try {
        setloading(true)
        const res = await fetch(`https://mern-blog-digr.onrender.com/api/post/getposts?userId=${currentuser._id}`)
  ...
  const delete_post = async (postid) => {
    ...
    try {
      ...
      const res = await fetch(`https://mern-blog-digr.onrender.com/api/post/delete/${postid}/${currentuser._id}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
